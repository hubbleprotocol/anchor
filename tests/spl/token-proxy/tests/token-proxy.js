...
const { splTokenProgram, SPL_TOKEN_PROGRAM_ID } = require("@coral-xyz/spl-token");

describe("program", () => {
  const provider = anchor.AnchorProvider.local();

  const TEST_PROGRAM_IDS = [SPL_TOKEN_PROGRAM_ID, new anchor.web3.PublicKey("TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb")];
  const TOKEN_PROGRAMS = TEST_PROGRAM_IDS.map(programId => splTokenProgram({
    provider,
    programId
  }));
  ...
  TOKEN_PROGRAMS.forEach(tokenProgram => {
    const name = tokenProgram.programId === SPL_TOKEN_PROGRAM_ID ? "token" : "token-2022";
    describe(name, () => {
      let mint = null;
      let from = null;
      let to = null;
      ...
      it("Initializes test state", async () => {
        ...
        from = await createTokenAccount(tokenProgram, mint, provider.wallet.publicKey);
        to = await createTokenAccount(tokenProgram, mint, provider.wallet.publicKey);
      });
...
async function createMint(tokenProgram) {
  ...
  const initMintIx = await tokenProgram.methods.initializeMint2(0, authority, null)
    .accounts({mint: mint.publicKey}).instruction();
...
async function createTokenAccount(tokenProgram, mint, owner) {
  ...
  const tx = new anchor.web3.Transaction();
  const createTokenAccountIx = await tokenProgram.account.account.createInstruction(vault);
  const initTokenAccountIx = await tokenProgram.methods.initializeAccount3(owner).accounts({account: vault.publicKey, mint}).instruction();
